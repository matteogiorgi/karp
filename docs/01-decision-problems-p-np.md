...
A **decision problem** is formalized as a language `L ⊆ Σ*`: a subset of the strings over a finite alphabet. An **instance** is a string `x`, and "solving" the problem on that instance means answering the question `x ∈ L?` with yes or no.
...
A language `L` is in **P** if there exists a deterministic algorithm that, for every instance `x`, decides `x ∈ L` in a number of steps bounded by `p(|x|)` for some fixed polynomial `p`.
...
> `L ∈ NP` if there exists a deterministic algorithm `V` (the **verifier**) and a polynomial `p` such that, for every `x`:
> `x ∈ L` **if and only if** there exists a string `y` (the **certificate**, or witness) with `|y| ≤ p(|x|)` such that `V(x, y)` accepts in time polynomial in `|x|`.
...
- **Independent Set**: certificate = the vertex set itself. Verification = check that no two chosen vertices are adjacent and that the size is `≥ k`, polynomial time in the number of chosen vertices.
...
The more common textbook definition is alternative but equivalent: `L ∈ NP` if there exists a nondeterministic Turing machine that decides `L` in polynomial time (i.e., for every instance in `L`, an accepting computation path of polynomial length *exists*).
...
`P ⊆ NP` is immediate from the verifier definition: if `L ∈ P`, the verifier can ignore the certificate entirely and decide `x ∈ L` on its own in polynomial time (equivalently: empty certificate, `V(x, y) := "solve x directly"`).

Whether the inclusion is strict (`P ≠ NP`) is the most famous open problem in theoretical computer science, and this project makes no attempt to say anything about it. It is worth being precise about what the report *implicitly* assumes: it works under the presupposition (universally believed, never proven) that `P ≠ NP`, because that presupposition is what makes it interesting for a problem to be NP-complete — if `P = NP` turned out to be true one day, every reduction built here would remain correct, but the very notion of "intrinsic difficulty" motivating them would collapse.

## 1.6 An asymmetry worth keeping in mind

The definition of NP is **asymmetric**: it talks about short certificates for yes-instances, and says nothing about how to quickly recognize no-instances. It is not obvious (and is itself an open problem related to P vs NP) that no-instances also have short certificates of their own negativity — this property defines the class **co-NP**, and whether `NP = co-NP` is unknown.

This asymmetry is not an erudite detail: it resurfaces exactly when, in the section on reductions, the correctness of a reduction `f` has to be argued in both directions, `x ∈ A ⟹ f(x) ∈ B` and `x ∉ A ⟹ f(x) ∉ B` — the second direction (preserving the *absence* of a solution) is typically the one that requires the real mathematical argument, because there is no certificate to exhibit to prove it for a single example.
