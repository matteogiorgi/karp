...
Given two decision problems `A, B ⊆ Σ*`, a **polynomial many-one reduction** (or **Karp reduction**) from `A` to `B` is a function `f: Σ* → Σ*` such that:

1. `f` is **total and computable in polynomial time** (a deterministic algorithm exists that computes `f(x)` in time `≤ p(|x|)` for every `x`, for some fixed polynomial `p`);
2. for every instance `x`: `x ∈ A ⟺ f(x) ∈ B`.

This is written `A ≤p B` ("`A` reduces to `B`"), and `f` is read as a **witness** of that relation: its mere existence is the proof that `A ≤p B` holds.
...
This project needs — and needs only — Karp's many-one notion, for a reason that goes beyond mere economy of means: **a single call, with no post-processing of the answer, is exactly the structure that makes reconstructing a certificate possible.** If `f(x) ∈ B` with certificate `y_B`, and the transformation `f` is known explicitly, one can often read `y_B` "backward" through the structure of `f` and obtain a certificate `y_A` for `x ∈ A` — this is precisely the role of the function `g` introduced in the project's code (`g: certificate_B → certificate_A`). With a generic Turing reduction, involving multiple adaptive queries and arbitrary combination logic, this reconstruction has in general no clean form: there is no single instance of `B` to "read backward" from.
