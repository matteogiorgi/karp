...
- the SAT oracle is `gophersat`, called in-process via its DIMACS-reading entry point — the DIMACS string built from `ThreeSAT` is handed straight to `gophersat`'s parser in memory, no subprocess and no temporary file, so "in-process" means *same binary*, not *native struct API*; the brute-force oracle is a single generic function (Go generics) parameterized over the instance type and the certificate type, reused for every problem other than `ThreeSAT`;
...
<p align="center">[§1](01-decision-problems-p-np.md) — [§2](02-purpose-of-complexity-classes.md) — [§3](03-polynomial-many-one-reductions.md) — [§4](04-purpose-of-reductions.md) — [§5](05-cook-levin-and-practice.md) — §6</p>
